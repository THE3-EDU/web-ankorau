...
export function createP5Sketch(config) {
  ...
  const sketch = new p5((p) => {
    ...
    class Magnet {
      ...
      touch(thumbx, thumby, indexx, indexy, middlex, middley, wristx, wristy) {
        ...
        if (!this.isSelected && !selectedMagnet && isPinching && isNear) {
          ...
          // 记录当前"深度"（中指尖-手腕距离）作为缩放基准
          if (middlex !== undefined && wristx !== undefined) {
            this.initialDepth = this.p.dist(middlex, middley, wristx, wristy);
          } else {
            this.initialDepth = 0;
          }
        ...
        if (this.isSelected) {
          if (isPinching) {
            // 用中指尖-手腕距离近似"手离镜头的远近"：近（距离大）放大，远（距离小）缩小
            let currentDepth = 0;
            if (middlex !== undefined && wristx !== undefined) {
              currentDepth = this.p.dist(middlex, middley, wristx, wristy);
            }
            ...
            if (this.initialDepth > 0 && currentDepth > 0) {
              let depthRatio = currentDepth / this.initialDepth;
              let targetW = this.initialSize.x * depthRatio;
              let targetH = this.initialSize.y * depthRatio;
              const minW = this.baseW * 0.7;
              const maxW = this.baseW * 5.0; // 全局最大 5 倍基础尺寸
              targetW = this.p.constrain(targetW, minW, maxW);
              const ratio = this.initialSize.y / this.initialSize.x;
            ...
            // 选中状态下，只要保持捏合就一直跟随手指，但避免与其他小马重叠
            if (Array.isArray(magnets)) {
              const candidateX = this.fingerx;
              const candidateY = this.fingery;
              let canMove = true;
              const selfRadius = Math.sqrt(this.w * this.w + this.h * this.h) / 2;
              magnets.forEach((other) => {
                if (!other || other === this || !other.pos) return;
                const otherRadius = Math.sqrt(other.w * other.w + other.h * other.h) / 2;
                const minDist = selfRadius + otherRadius * 0.9; // 稍微保留一点间距
                const d = this.p.dist(candidateX, candidateY, other.pos.x, other.pos.y);
                if (d < minDist) {
                  canMove = false;
                }
              });
              if (canMove) {
                this.pos.x = candidateX;
                this.pos.y = candidateY;
              }
            } else {
              this.pos.x = this.fingerx;
              this.pos.y = this.fingery;
