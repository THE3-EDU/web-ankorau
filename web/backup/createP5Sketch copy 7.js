  ...
  const sketch = new p5((p) => {
    ...
    class Magnet {
      ...
      touch(thumbx, thumby, indexx, indexy) {
        ...
        // 适度放宽选中范围（1.5 倍半径），但仍需靠近，不再远距离吸附
        const isNear = distFromFingers < detectionRadius * 1.5;
        const isPinching = distBetweenFingers < 150; // 手指捏合阈值
        ...
        if (this.isSelected) {
          if (isPinching) {
            ...
            if (this.initialDistance > 0 && distBetweenFingers > 0) {
              ...
              // 限制缩放范围：最小 0.7 倍，最大 8 倍
              scaleFactor = this.p.constrain(scaleFactor, 0.7, 5.0);
    ...
    p.setup = () => {
      ...
      magnetImg = p.loadImage(magnetUrl, (img) => {
    ...
    p.draw = () => {
      ...
      if (video && video.elt) {
        ...
        if (!window.isHandPoseReady) {
          ...
          p.fill(0, 150);
          // 中间一块半透明黑色背景
          const loadingW = canvasW * 0.6;
          const loadingH = 60;
          const loadingX = (canvasW - loadingW) / 2;
          const loadingY = (canvasH - loadingH) / 2;
          p.rect(loadingX, loadingY, loadingW, loadingH, 8);
          p.fill(255);
          p.textAlign(p.CENTER, p.CENTER);
          p.textSize(20);
          p.text('手势识别加载中…', canvasW / 2, canvasH / 2);
          p.pop();
        }

        // 绘制手部关键点（优化：降低绘制频率，减少性能开销）
        // 每帧绘制一次（保持视觉流畅性）
                  ...
                  if (thumbCanvas && indexCanvas) {
                    magnets.forEach((m) => {
                      if (m && typeof m.touch === 'function') {
                        m.touch(thumbCanvas.x, thumbCanvas.y, indexCanvas.x, indexCanvas.y);
                      }
                    });
                  }
                }
          ...
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
            ...
            if (magnets && magnets.length > 0) {
              magnets.forEach((m) => {
                ...
                recordCtx.drawImage(src, -m.w / 2, -m.h / 2, m.w, m.h);
                recordCtx.restore();
              });
            }
          }
