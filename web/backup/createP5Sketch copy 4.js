  ...
  const sketch = new p5((p) => {
            ...
            window.handsSolution.onResults((results) => {
                try {
                    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
                      window.hands = results.multiHandLandmarks.map((landmarks, index) => {
                        const handData = {
                        ...
                        if (results.multiHandedness && results.multiHandedness[index]) {
                          handData.handedness = results.multiHandedness[index].categoryName;
                          handData.score = results.multiHandedness[index].score;
                        }
                    ...
                    console.error('onResults 处理错误:', err);
                    window.hands = [];
                  }
                });
    ...
    p.draw = () => {
      if (video && video.loadedmetadata) {
        ...
        // 绘制手部关键点和连接线
        if (window.hands && window.hands.length > 0 && video && video.elt) {
          const videoEl = video.elt;
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
        ...
        if (window.isHandPoseReady && window.handsSolution && video && video.elt) {
          // 每 3 帧调用一次，避免过于频繁
          if (p.frameCount % 3 === 0) {
            // console.log('发送数据', window.handsSolution);
            window.handsSolution.send({ image: videoEl }).catch(e => console.warn('Hands send skipped', e));
          }
        }
      }
    };
