...
export function createP5Sketch(config) {
  ...
  const sketch = new p5((p) => {
    ...
    function drawToRecordCanvas() {
      ...
      // 注意：主画布上视频和 GIF 都是镜像的，而录制画布上视频没有镜像
      // 所以需要将 GIF 的坐标转换回原始坐标，并翻转 GIF 本身
      if (magnets && magnets.length > 0) {
        magnets.forEach((m) => {
          ...
          recordCtx.save();
          // 将镜像后的坐标转换回原始坐标（因为主画布上 m.pos.x 已经是镜像后的）
          const originalX = recordCanvasW - m.pos.x;
          // 移动到位置
          recordCtx.translate(originalX, m.pos.y);
          // 先水平翻转 GIF（因为视频没有镜像）
          recordCtx.scale(-1, 1);
          // 然后旋转
    ...
    async function handleRecordingComplete() {
      ...
      console.log('[录制完成] 预览 URL 已设置，应该显示预览弹窗');
