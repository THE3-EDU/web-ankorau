        ...
        const onVideoReady = async () => {
          ...
          if (!handsInitialized && videoEl.videoWidth > 0) {
            ...
            try {
              ...
              // 如果已经存在，先清理（防止页面刷新时的残留）
              if (handsSolution) {
                try {
                  // 尝试清理旧的实例
                  if (typeof handsSolution.close === 'function') {
                    handsSolution.close();
                  }
                } catch (e) {
                  // 忽略清理错误
                ...
                handsSolution.onResults(function(results) {
                  try {
                    ...
                    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
                      hands = results.multiHandLandmarks.map(function(landmarks, index) {
                        const handData = {
                          landmarks: landmarks.map(function(lm) { return [lm.x, lm.y, lm.z]; }),
                          keypoints: landmarks.map(function(lm) { return { x: lm.x, y: lm.y, z: lm.z }; })
                        };
                      ...
                      console.log('检测到手，数量:', hands.length);
                    } else {
                      hands = [];
                    }
                  } catch (error) {
