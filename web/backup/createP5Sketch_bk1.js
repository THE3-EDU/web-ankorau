  ...
  const sketch = new p5((p) => {
    ...
    p.draw = () => {
      ...
      if (video && video.elt) {
        ...
        // 每 2 帧绘制一次（保持视觉流畅性）
        if (window.hands && window.hands.length > 0 && video && video.elt && p.frameCount % 1 === 0) {
          const videoEl = video.elt;
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
        ...
        if (recordCanvas && recordCtx && video && video.elt) {
          ...
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
            ...
            if (window.hands && window.hands.length > 0 && recordCtx && p.frameCount % 1 === 0) {
        ...
        // 非录制时：每 3 帧调用一次（约 20fps 检测频率，平衡性能和响应速度）
        if (window.isHandPoseReady && window.handsSolution && video && video.elt) {
          const detectInterval = 3;
          if (p.frameCount % detectInterval === 0) {
            try {
              // 使用 video.elt 而不是 videoEl（确保变量在作用域内）
              const currentVideoEl = video.elt;
              if (currentVideoEl) {
                window.handsSolution.send({ image: currentVideoEl }).catch(e => {
                  // 静默处理错误，避免控制台噪音
        ...
        if (p.frameCount % 3 === 0) {
          ...
          // 先画一条半透明背景条，避免文字重影
          p.fill(0, 150);
          p.rect(0, canvasH - 30, canvasW, 30);
          // 再画白色文字
          p.fill(255);
          p.textAlign(p.RIGHT, p.BOTTOM);
          p.textSize(16);
          p.text(`FPS: ${fps}`, canvasW - 12, canvasH - 10);
          p.pop();
        }
      }
    };
