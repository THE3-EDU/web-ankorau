...
if (!window.handsInitialized) window.handsInitialized = false;
if (!window.handsSolution) window.handsSolution = null;
if (!window.isHandPoseReady) window.isHandPoseReady= false;
if (!window.hands) window.hands = [];
  ...
  const sketch = new p5((p) => {
    ...
    p.setup = () => {
      ...
      if (video && video.elt) {
        ...
        const initHandsOnce = () => {
            if (window.handsInitialized) return;
            window.handsInitialized = true;
          
            if (window.handsSolution) return;
    ...
    p.draw = () => {
      if (video && video.loadedmetadata) {
        ...
        // 测试绘制功能：在画布右上角绘制一个黄色测试点（已注释，避免干扰）
        // p.fill(255, 255, 0);
        // p.noStroke();
        ...
        if (recordCanvas && recordCtx && video && video.elt) {
          ...
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
            ...
            if (window.hands && window.hands.length > 0 && recordCtx) {
        ...
        // 每 3 帧调用一次（约 20fps 检测频率，平衡性能和响应速度）
        if (window.isHandPoseReady && window.handsSolution && video && video.elt) {
          if (p.frameCount % 3 === 0) {
            try {
              window.handsSolution.send({ image: videoEl }).catch(e => {
                // 静默处理错误，避免控制台噪音
