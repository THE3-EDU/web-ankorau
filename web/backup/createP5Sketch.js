...
export function createP5Sketch(config) {
  ...
  const sketch = new p5((p) => {
    ...
    let detectCtx = null;
    // 再降低检测分辨率（4:3 比例），进一步减轻手机端负担
    // 192x144 只承担检测，不影响主画面显示清晰度
    const detectCanvasW = 192;
    const detectCanvasH = 144;

    p.setup = () => {
      // 降低像素密度，避免在高 DPR 手机上实际渲染过多像素
      // 视觉尺寸不变，但 GPU/CPU 负担会明显减轻
      if (typeof p.pixelDensity === 'function') {
        p.pixelDensity(1);
      }
    ...
    p.draw = () => {
      ...
      if (video && video.elt) {
        ...
        // 每 2 帧绘制一次（保持视觉流畅性）
        if (window.hands && window.hands.length > 0 && video && video.elt && p.frameCount % 2 === 0) {
          const videoEl = video.elt;
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
              ...
              if (hand && Array.isArray(points) && points.length > 0) {
                ...
                connections.forEach(function(conn) {
                  ...
                  if (points && points[start] && points[end]) {
                    ...
                    // 检查坐标有效性
                    if (startX === undefined || startY === undefined || isNaN(startX) || isNaN(startY)) {
                      return;
                    }
                    ...
                    if (startX >= 0 && startX <= 1 && startY >= 0 && startY <= 1) {
                      // 使用预计算的缩放因子
                      canvasStartX = displayOffsetX + startX * displayW;
                      canvasStartY = displayOffsetY + startY * displayH;
                    } else {
                      canvasStartX = displayOffsetX + startX * scaleX;
                      canvasStartY = displayOffsetY + startY * scaleY;
                    }
                    
                    // 检查画布坐标有效性，避免绘制到 (0, 0)
                    if (canvasStartX < 0 || canvasStartX > canvasW || canvasStartY < 0 || canvasStartY > canvasH) {
                      return;
                    }
                    ...
                    if (endX >= 0 && endX <= 1 && endY >= 0 && endY <= 1) {
                      // 使用预计算的缩放因子
                      canvasEndX = displayOffsetX + endX * displayW;
                      canvasEndY = displayOffsetY + endY * displayH;
                    } else {
                      canvasEndX = displayOffsetX + endX * scaleX;
                      canvasEndY = displayOffsetY + endY * scaleY;
                    }
                    
                    // 检查画布坐标有效性，避免绘制到 (0, 0)
                    if (canvasEndX < 0 || canvasEndX > canvasW || canvasEndY < 0 || canvasEndY > canvasH) {
                      return;
                    }
                    
                    // 绘制连接线（绿色，优化：减小线条粗细）
                    p.stroke(0, 255, 0);
                    p.strokeWeight(1.5); // 减小线条粗细，提升性能
        ...
        if (recordCanvas && recordCtx && video && video.elt) {
          ...
          if (videoEl && videoEl.videoWidth && videoEl.videoHeight) {
            ...
            // 录制时绘制关键点和连接线，每 2 帧绘制一次，降低绘制频率
            if (window.hands && window.hands.length > 0 && recordCtx && p.frameCount % 2 === 0) {
                window.hands.forEach((hand) => {
                ...
                if (hand && Array.isArray(points) && points.length > 0 && recordCtx) {
                  ...
                  points.forEach((point) => {
                    ...
                    // 坐标转换（使用预计算的缩放因子，优化性能）
                    let canvasX;
                    let canvasY;
                    
                    if (x >= 0 && x <= 1 && y >= 0 && y <= 1) {
                      // 归一化坐标：使用预计算的缩放因子
                      canvasX = offsetX + x * drawW;
                      canvasY = offsetY + y * drawH;
                    } else {
                      // 像素坐标：使用预计算的缩放因子
                    ...
                    // 绘制关键点
                    recordCtx.fillStyle = 'rgba(255, 0, 0, 0.8)';
                    recordCtx.beginPath();
                    recordCtx.arc(canvasX, canvasY, 4, 0, 2 * Math.PI); // 减小圆圈尺寸
                    recordCtx.fill();
                  });
                  
                  // 绘制手部连接线（绿色）
                  recordCtx.strokeStyle = 'rgba(0, 255, 0, 0.6)';
                  ...
                  connections.forEach((conn) => {
                    ...
                    if (points && points[start] && points[end]) {
                      ...
                      if (startX >= 0 && startX <= 1 && startY >= 0 && startY <= 1) {
                        // 归一化坐标
                        canvasStartX = offsetX + startX * drawW;
                        canvasStartY = offsetY + startY * drawH;
                      } else {
                        // 像素坐标
                        canvasStartX = offsetX + startX * scaleX;
                        canvasStartY = offsetY + startY * scaleY;
                      }
                      
                      // 检查画布坐标有效性
                      if (canvasStartX < 0 || canvasStartX > recordCanvasW || canvasStartY < 0 || canvasStartY > recordCanvasH) {
                        return;
                      }
                      ...
                      if (endX >= 0 && endX <= 1 && endY >= 0 && endY <= 1) {
                        // 归一化坐标
                        canvasEndX = offsetX + endX * drawW;
                        canvasEndY = offsetY + endY * drawH;
                      } else {
                        // 像素坐标
                        canvasEndX = offsetX + endX * scaleX;
                        canvasEndY = offsetY + endY * scaleY;
                      }
                      
                      // 检查画布坐标有效性
                      if (canvasEndX < 0 || canvasEndX > recordCanvasW || canvasEndY < 0 || canvasEndY > recordCanvasH) {
                        return;
                      }
        ...
        // 录制时：每 6 帧调用一次（降低检测频率，减少性能开销）
        // 非录制时：每 4 帧调用一次（约 15fps 检测频率，进一步提升整体帧率）
        if (window.isHandPoseReady && window.handsSolution && video && video.elt && detectCanvas && detectCtx) {
          const detectInterval = 4;
        ...
        // 在画面下方显示当前帧率（FPS），每 3 帧更新一次文本，减少重绘开销
        if (p.frameCount % 3 === 0) {
          ...
          // 先画一条半透明背景条，避免文字重影
          p.fill(0, 150);
          p.rect(0, 30, canvasW, 30);
          // 再画白色文字
          p.fill(255);
          p.textAlign(p.RIGHT, p.BOTTOM);
          p.textSize(16);
          p.text(`FPS: ${fps}`, canvasW - 12, 60);
          p.pop();
        }
      }
    };
